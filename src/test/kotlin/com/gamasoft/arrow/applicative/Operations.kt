...
import arrow.core.*
import arrow.data.*
import arrow.data.ListKW.Companion.empty
import arrow.data.ListKW.Companion.pure
...
typealias UValidated<E, A> = Validated<ListKW<E>, A>
typealias UValid<A> = Validated.Valid<A>
typealias UInvalid<E> = Validated.Invalid<ListKW<E>>
...
fun applicativeValidation() {
    ...
    val fs = ListKW(listOf(positive, even, smallerThan10))

    val v1 = ListKW.pure(13)
    val errors = v1.ap(fs).flatMap{ x -> if (x is Either.Left) pure(x.a) else empty() }.list
    ...
    fun Int.validateAgainst(rules: ListKW<IntValidation>): ValidatedNel<String, Int> = // rules.map { it(this) }.fold()
